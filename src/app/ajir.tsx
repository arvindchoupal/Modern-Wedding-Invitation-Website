...
import { AjitWeddingCard } from "./components/ajitWedding";
import { AjitStoryReveal } from "./components/storyRevealAjit";
export default function Ajit() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-red-50 to-orange-50">
      ...
      {!cardOpened ? (
        <AjitWeddingCard onOpen={handleCardOpen} />
      ) : (
        <AjitStoryReveal onBack={() => setCardOpened(false)} />
      )}
    </div>
  );
}